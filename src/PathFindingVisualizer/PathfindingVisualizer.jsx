...
import {
   getNewGridWithWallToggled, getInitialGrid, animateGivenNode } from '../lib/gridFunctions';
...
const PathfindingVisualizer = () => {
  ...
  const handleMouseEnter = (row, col) => {
    if(isChangingStartNode){
      setStartNode(prev => {return {row: row, col:col}});
      changeNode(row,col,"start");
    }else if(isChangingFinishNode){
      setFinishNode(prev => {return {row: row, col:col}});
...
const changeNode = (row,col,type ) => {
  ...
  const animateDijkstra = (visitedNodesInOrder, nodesInShortestPathOrder) => {
    for (let i = 0; i <= visitedNodesInOrder.length; i++) {
      ...
      setTimeout(() => {
        ...
        animateGivenNode(node,"node node-visited", setGrid);
  ...
  const animateShortestPath = (nodesInShortestPathOrder) => {
    ...
    for (let i = 0; i < nodesInShortestPathOrder.length; i++) {
      ...
      setTimeout(() => {
        ...
        animateGivenNode(node,"node node-shortest-path", setGrid);
      }, 30 * i);
  ...
  const visualizeDijkstra =() => {
    if(grid[startNode.row][startNode.col].isWall || grid[finishNode.row][finishNode.col].isWall){
    ...
    const visitedNodesInOrder = dijkstra(grid, latestStartNode, latestFinishNode);
    const nodesInShortestPathOrder = getNodesInShortestPathOrder(latestFinishNode);
    animateDijkstra(visitedNodesInOrder, nodesInShortestPathOrder);
    };
    return (
        <>
        <Header visualizeDijkstra={visualizeDijkstra} isRunning={isRunning} setGrid={setGrid} grid={grid} getInitialGrid={()=>getInitialGrid(startNode,finishNode)} />
          <div className="grid">
            {grid.map((row, rowIdx) => {
              return (
                <div key={rowIdx} className="row">
                  {row.map((node, nodeIdx) => {
                    const {row, col, isFinish, isStart, isWall, className} = node;
                    return (
                      <Node
                      className={className}
                        key={nodeIdx}
                        ...
                        isStart={isStart}
                        isWall={isWall}
                        onClick={(row, col) => { setStartNode({row, col})}}
